#include <onlp/platformi/ledi.h>
#include <sys/mman.h>
#include <stdio.h>
#include <string.h>
#include <fcntl.h>
#include <unistd.h>
#include <time.h>

#include "platform.h"

enum onlp_led_id
{
    LED_RESERVED = 0,
    LED_SYSTEM,
    LED_ALARM,
    LED_PSU,
    LED_FAN
};

/*
 * Get the information for the given LED OID.
 */
static onlp_led_info_t led_info[] =
{
    { },
    {
        { ONLP_LED_ID_CREATE(LED_SYSTEM), "System LED (Front)", 0 },
        ONLP_LED_STATUS_PRESENT,
        ONLP_LED_CAPS_ON_OFF | ONLP_LED_CAPS_YELLOW | ONLP_LED_CAPS_YELLOW_BLINKING | ONLP_LED_CAPS_GREEN | 
        ONLP_LED_CAPS_GREEN_BLINKING | ONLP_LED_CAPS_AUTO,
    },
    {
        { ONLP_LED_ID_CREATE(LED_ALARM), "Alert LED (Front)", 0 },
        ONLP_LED_STATUS_PRESENT, ONLP_LED_CAPS_ON_OFF  | ONLP_LED_CAPS_YELLOW_BLINKING | ONLP_LED_CAPS_GREEN 
    }, //no (  ONLP_LED_CAPS_YELLOW | ONLP_LED_CAPS_GREEN_BLINKING) support in cloverstone_dp
    {
        { ONLP_LED_ID_CREATE(LED_PSU), "PSU LED (Front)", 0 },
        ONLP_LED_STATUS_PRESENT,
        ONLP_LED_CAPS_ON_OFF | ONLP_LED_CAPS_AUTO | ONLP_LED_CAPS_YELLOW | ONLP_LED_CAPS_GREEN,
    },
    {
        { ONLP_LED_ID_CREATE(LED_FAN), "FAN LED (Front)", 0 },
        ONLP_LED_STATUS_PRESENT,
        ONLP_LED_CAPS_ON_OFF | ONLP_LED_CAPS_AUTO | ONLP_LED_CAPS_YELLOW | ONLP_LED_CAPS_GREEN,
    }
};

int
onlp_ledi_init(void)
{
    return ONLP_STATUS_OK;
}

// modify to suit sys cpld register
#if 0 
int
onlp_ledi_info_get(onlp_oid_t id, onlp_led_info_t* info_p)
{
    int led_id;
    uint8_t led_color = 0;
    uint8_t blink_status = 0;
    uint8_t hw_control_status = 0;
    uint8_t result = 0;

    led_id = ONLP_OID_ID_GET(id);
    *info_p = led_info[led_id];
 
    result = get_led_status(led_id);

    if(result != 0xFF)
        info_p->status |= ONLP_LED_STATUS_ON;

    switch(led_id){
        case LED_SYSTEM:
        case LED_ALARM:

            led_color = (result >> 4)&0x3;
            if(led_color == 0){
                if(led_id==LED_SYSTEM)
                    info_p->mode |= ONLP_LED_MODE_AUTO;
                if(led_id==LED_ALARM)
                    info_p->mode |= ONLP_LED_MODE_OFF;
            }
            if(led_color == 1){
                info_p->mode |= ONLP_LED_MODE_GREEN;
            }
            if(led_color == 2){
                info_p->mode |= ONLP_LED_MODE_YELLOW;
            }
            if(led_color == 3){
                info_p->mode |= ONLP_LED_MODE_OFF;
                break;
            }

            blink_status = result & 0x3;
            if(blink_status == 1 || blink_status == 2){
                int current_mode = info_p->mode;
                info_p->mode = current_mode+1;
            }

            break;
        case LED_PSU:
        case LED_FAN:
            hw_control_status = (result >> 4) & 0x1;
            led_color = result & 0x3;
            if(!hw_control_status)
            {
                if(led_color == 1){
                    info_p->mode = ONLP_LED_MODE_YELLOW;
                }else if(led_color == 2){
                    info_p->mode = ONLP_LED_MODE_GREEN;
                }else if(led_color == 3){
                    info_p->mode = ONLP_LED_MODE_OFF;
                }
            }else{
                info_p->mode = ONLP_LED_MODE_AUTO;
            }
            break;
    }
    return ONLP_STATUS_OK;
}
#endif

// according to sys cpld registers definition
int
onlp_ledi_info_get(onlp_oid_t id, onlp_led_info_t* info_p)
{
    int led_id;
    uint8_t led_color = 0;
    uint8_t blink_status = 0;
    uint8_t result = 0;
    int ret = 0;

    led_id = ONLP_OID_ID_GET(id);
    *info_p = led_info[led_id];
    
    ret = onlp_lpc_connect(LPC_CONNECT_COME);
    if(ret != 0){
        return ret;
    }
    result = get_led_status(led_id);
    if(result != 0xFF)
        info_p->status |= ONLP_LED_STATUS_ON;

    switch(led_id){
        case LED_SYSTEM:
            led_color = (result >> 4)&0x3;
            if(led_color == 0){
                info_p->mode |= ONLP_LED_MODE_AUTO;
            }
            if(led_color == 1){
                info_p->mode |= ONLP_LED_MODE_GREEN;
            }
            if(led_color == 2){
                info_p->mode |= ONLP_LED_MODE_YELLOW;
            }
            if(led_color == 3){
                info_p->mode |= ONLP_LED_MODE_OFF;
                break;
            }

            blink_status = result & 0x3;
            if(blink_status == 1 || blink_status == 2){
                int current_mode = info_p->mode;
                info_p->mode = current_mode+1;
            }

            break;
        case LED_ALARM:
            led_color = (result >> 5) & 0x3;
            if(led_color == 0){
                info_p->mode |= ONLP_LED_MODE_OFF;
            }
            if(led_color == 1){
                info_p->mode |= ONLP_LED_MODE_GREEN;
            }
            if(led_color == 2 || led_color == 3){
                info_p->mode |= ONLP_LED_MODE_YELLOW_BLINKING;
            }
            
            break;
        case LED_PSU:
            led_color = (result >> 1) & 0x1;
            if(led_color == 0){
                led_color = result & 0x1;
                if(led_color == 0){
                    info_p->mode |= ONLP_LED_MODE_OFF;
                }else if(led_color == 0){
                    info_p->mode |= ONLP_LED_MODE_YELLOW;
                }
            }
            else if(led_color == 1){
                info_p->mode |= ONLP_LED_MODE_YELLOW;
            }
        case LED_FAN:
            led_color = (result >> 3) & 0x1;
            if(led_color == 0){
                led_color = (result >> 2) & 0x1;
                if(led_color == 0){
                    info_p->mode |= ONLP_LED_MODE_OFF;
                }else if(led_color == 0){
                    info_p->mode |= ONLP_LED_MODE_YELLOW;
                }
            }
            else if(led_color == 1){
                info_p->mode |= ONLP_LED_MODE_YELLOW;
            }
            
            break;
    }
    ret = onlp_lpc_connect(LPC_CONNECT_CMMBMC); // recover the init status
    if(ret != 0){
        return ret;
    }
    return ONLP_STATUS_OK;
}



