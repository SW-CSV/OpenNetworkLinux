...
int onlp_lpc_connect(int mode)
{
    /* add here to switch lpc to link with COMe according to sys cpld */
    char command[256];
    FILE *fp;
    if (mode == LPC_CONNECT_COME){ // connect to COMe
        sprintf(command, "i2cset -y -f 0 0x70 0x00"); // 0x70 as i2c selector, close all channel here
        fp = popen(command, "r");
        ...
        sprintf(command, "i2cset -y -f 0 0x70 0x20"); // 0x70 as i2c selector, select sys cpld i2c channel here
        fp = popen(command, "r");
        usleep(5000); //Sleep for 5 Microsec for waiting the file operation complete
        if (!fp)
        {
        ...
        sprintf(command, "i2cset -y -f 0 0x59 0x3d 0x00"); // 0x59 as sys cpld, select sys cpld i2c lpc linking to COMe here.
        fp = popen(command, "r");
        usleep(5000); //Sleep for 5 Microsec for waiting the file operation complete
        if (!fp)
        {
    ...
    }else if(mode == LPC_CONNECT_CMMBMC){
        sprintf(command, "i2cset -y -f 0 0x59 0x3d 0x02"); // 0x59 as sys cpld, select sys cpld i2c lpc linking to CMM BMC here.
        fp = popen(command, "r");
        usleep(5000); //Sleep for 5 Microsec for waiting the file operation complete
        if (!fp)
        {
...
uint8_t get_psu_status(int id)
{
    ...
    char *token = NULL;
    int len = 0;
    char *psu_sensor_name[PSU_COUNT] = {
        "PSU1","PSU2","PSU3"};
    ...
    ...
    PSU1 STATUS      | 0x0        | discrete   | 0x0180| na        | na        | na        | na        | na        | na
    PSU2_STATUS      | 0x0        | discrete   | 0x0180| na        | na        | na        | na        | na        | na
    PSU3_STATUS      | 0x0        | discrete   | 0x0180| na        | na        | na        | na        | na        | na
    ...

    0x0180's 0x01 bit0 1--present, bit3 1--ac_state lost, bit5 1--power_state abnormal, according to ipmi protocol
    */
    content = strtok_r(tmp, "\n", &temp_pointer);

    while(content != NULL){
        if (strstr(content, psu_sensor_name[id - 1])) {
            token = strtok(content, "|");
            token = strtok(NULL, "|");
            token = strtok(NULL, "|");
            token = strtok(NULL, "|");
            char* trim_token = trim(token);
            sscanf(trim_token, "%x", &res);
        }
    ...
    ret = (uint8_t)(~(res >> 8) & 0xff); // need to reverse because ipmi protocol differs from previous cpld definition.
    return ret;
}
...
int get_psu_info(int id, int *mvin, int *mvout, int *mpin, int *mpout, int *miin, int *miout)
{
    ...
    */
    ...
    else if(id == 2)
    {
        ...
        if (0 == strcmp(&strTmp[12][1], "na"))
            *mvin = 0;
        else
            *mvin = atof(&strTmp[12][1]) * 1000.0;

        if (0 == strcmp(&strTmp[15][1], "na"))
            *mvout = 0;
        else
            *mvout = atof(&strTmp[15][1]) * 1000.0;

        if (0 == strcmp(&strTmp[14][1], "na"))
            *mpin = 0;
        else
            *mpin = atof(&strTmp[14][1]) * 1000.0;

        if (0 == strcmp(&strTmp[17][1], "na"))
            *mpout = 0;
        else
            *mpout = atof(&strTmp[17][1]) * 1000.0;

        if (0 == strcmp(&strTmp[13][1], "na"))
            *miin = 0;
        else
            *miin = atof(&strTmp[13][1]) * 1000.0;

        if (0 == strcmp(&strTmp[16][1], "na"))
            *miout = 0;
        else
            *miout = atof(&strTmp[16][1]) * 1000.0;
    }
...
int get_fan_info(int id, char *model, char *serial, int *isfanb2f)
{
    ...
    index = (id + 1) / 2; // /2 because Fan1 Front & Rear use the same FRU_FAN info
    strcpy(model, fan_information[index].model);
    strcpy(serial, fan_information[index].serial_number);
    *isfanb2f = fan_information[index].airflow;
...
int get_sensor_info(int id, int *temp, int *warn, int *error, int *shutdown)
{
    ...
    char *Thermal_sensor_name[THERMAL_COUNT] = {
        "TEMP_CPU", "TMP_CMM_U31","TMP_CMM_U32", "TMP_SCM_U35", "TMP_SCM_U37",
        "TMP_LC1_U85_L", "TMP_LC1_U85_R1","TMP_LC1_U85_R2","TMP_LC1_U18","TMP_LC1_U22",
        "TMP_LC1_U23", "TMP_LC2_U85_L", "TMP_LC2_U85_R1","TMP_LC2_U85_R2","TMP_LC2_U18",
        "TMP_LC2_U22", "TMP_LC2_U23", "PSU1_TEMP1","PSU1_TEMP2", "PSU1_TEMP3", "PSU2_TEMP1",
        "PSU2_TEMP2", "PSU2_TEMP3", "PSU3_TEMP1", "PSU3_TEMP2", "PSU3_TEMP3"};
...
int get_fan_speed(int id,int *per, int *rpm)
{
    ...
    int max_rpm_speed = 15000;// = 100% speed(Delta)
    char *tmp = (char *)NULL;
    int len = 0;
    int index  = 0;
    ...
    char *token = NULL;
    char *Fan_sensor_name[FAN_COUNT] = {
        "FAN1_FRONT", "FAN1_REAR", "FAN2_FRONT", "FAN2_REAR", "FAN3_FRONT", "FAN3_REAR",
        "FAN4_FRONT", "FAN4_REAR", "FAN5_FRONT", "FAN5_REAR", "PSU1_FAN","PSU2_FAN","PSU3_FAN"};
    ...
    /*
        ...
        FAN1_FRONT       | 7875.000   | RPM        | ok    | na        | 1050.000  | na        | na        | na        | na
        FAN1_REAR        | 7425.000   | RPM        | ok    | na        | 1050.000  | na        | na        | na        | na
        FAN2_FRONT       | 7875.000   | RPM        | ok    | na        | 1050.000  | na        | na        | na        | na
        FAN2_REAR        | 7425.000   | RPM        | ok    | na        | 1050.000  | na        | na        | na        | na
        FAN3_FRONT       | 7950.000   | RPM        | ok    | na        | 1050.000  | na        | na        | na        | na
        FAN3_REAR        | 7425.000   | RPM        | ok    | na        | 1050.000  | na        | na        | na        | na
        FAN4_FRONT       | 7950.000   | RPM        | ok    | na        | 1050.000  | na        | na        | na        | na
        FAN4_REAR        | 7425.000   | RPM        | ok    | na        | 1050.000  | na        | na        | na        | na
        FAN5_FRONT       | 7875.000   | RPM        | ok    | na        | 1050.000  | na        | na        | na        | na
        FAN5_REAR        | 7350.000   | RPM        | ok    | na        | 1050.000  | na        | na        | na        | na
        PSU1_FAN         | na         | RPM        | na    | na        | na        | na        | na        | na        | na
        PSU2_FAN         | na         | RPM        | na    | na        | na        | na        | na        | na        | na
        PSU3_FAN         | na         | RPM        | na    | na        | na        | na        | na        | na        | na
    */
    if(is_shm_mem_ready()){
        ret = open_file(ONLP_SENSOR_LIST_CACHE_SHARED,ONLP_SENSOR_LIST_SEM, &tmp, &len);
        if(ret < 0 || !tmp){
